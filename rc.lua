...
if editor == "subl" then
    ...
    editor_cmd = terminal .. " -x sh -c " .. editor
end
...
hdmi_duplicate = "xrandr --output DisplayPort-2 --mode 1366x768  --set underscan off --same-as LVDS"
hdmi_extend    = "xrandr --output DisplayPort-2 --mode 1920x1080 --set underscan on  --above   LVDS"
xrandr_auto    = "xrandr --auto"

myhdmimenu = {
   { "&stop xrandr", xrandr_auto },
   { "&duplicate (hdmi)", hdmi_duplicate },
   { "&extend (hdmi)", hdmi_extend }
}

mymainmenu = awful.menu({ items = { { "&awesome", myawesomemenu, beautiful.awesome_icon },
                                    { "&display", myhdmimenu },
                                    { "open &terminal", terminal, beautiful.terminal_icon },
                                    { "&firefox", browser, beautiful.browser_icon }
                                  }
                        })
...
brightup   = function() awful.util.spawn("light -s " .. backlight .. " -r -A " .. brightstep, false) end
brightdown = function() awful.util.spawn("light -s " .. backlight .. " -r -U " .. brightstep, false) end
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({ "main", "other" }, s, { awful.layout.layouts[1], awful.layout.layouts[4] })
...
globalkeys = awful.util.table.join(
    ...
    awful.key({}, "XF86Battery", hibernate,
              {description = "hibernate", group = "multimedia"}),
    awful.key({}, "XF86Sleep", suspend,
        {description = "suspend", group = "multimedia"}),
